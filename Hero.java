...
public class Hero extends Entity implements Fighter, Magical, Archer {
  ...
  public void levelUp() {
    this.level += 1;
    Map.getInstance().loadMap((this.level-1)%3 + 1);
   ...
   * @param atkChoice passes their choice of attack
   * @return String returns the string represeting the damage
   */
  public String attack(Enemy e, int choice, int atkChoice) {
    String toReturn = "";
    switch (choice) {
      case 1: 
      if (atkChoice == 1) toReturn =  this.sword(e);
      else toReturn =  this.axe(e);
      break;
      case 2: 
      if (atkChoice == 1) toReturn =  this.magicMissile(e);
      else toReturn =  this.fireball(e);
      break;
      case 3: 
      if (atkChoice == 1) toReturn =  this.arrow(e);
    ...
    if (e.getHp() <= 0) {
      ...
      int droppedGold =  (int)(Math.random() * ((Math.abs(e.getHp())) + 3)) + 1 * level;
  ...
  public String sword(Entity e) {
    int damage = (int) (Math.random() * 2)+1; // 1-2
    e.takeDamage(damage);
  ...
  public String axe(Entity e) {
    int damage = (int) (Math.random() * 4) + 1; // 0-4
    e.takeDamage(damage);
    return this.getName() + " slashes " + e.getName() + " for " + damage + " damage.";
  }
  ...
  int damage = (int) (Math.random() * 3)+1; // 1-3
  e.takeDamage(damage);
  ...
  public String fireball(Entity e) {
    int damage = (int) (Math.random() * 5) + 1; // 1-5
    e.takeDamage(damage);
  ...
  public String arrow(Entity e) {
    int damage = (int) (Math.random() * 5); // 0-4
    e.takeDamage(damage);
  ...
  public String fireArrow(Entity e) {
    int damage = (int) (Math.random() * 5) + 1; // 1-5
    e.takeDamage(damage);
    return this.getName() + " explodes " + e.getName() + " with a fire arrow for " + damage + " damage.";
  }
}
